...
def Parse_Args():
    parser = argparse.ArgumentParser(prog='MultiCNASingle', description="Creates multiple CNA heatmaps of varying bin sizes for a given set of bam files that are single end")
            ...
            try:
                if os.path.isfile(csvoutput+str(i)+'.csv') is False:
                    if mode=='SECNV':
                        ...
                        os.system('Rscript $GINISingle '+str(i)[:-3]+' '+seoutput)
                        Gini=GetGini(seoutput+'gini.csv')
                        os.system('rm '+seoutput+'gini.csv && Rscript $SCOPESingle '+str(i)[:-3]+' '+str(Gini))
